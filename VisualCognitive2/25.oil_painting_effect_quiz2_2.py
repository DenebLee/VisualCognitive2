...
def oil_painting_effect(src, radius, levels):
	...
	for i in range(src.shape[0]):
		...
		for j in range(src.shape[1]):
			...
			freqMaxIdx = np.argmax(freq)
			max_bin = idx[freqMaxIdx]
			bgr_sum[max_bin] = roiBGR.reshape((-1,3))[roiHist.ravel() == max_bin].sum(axis=0)
			# error because max_bin is not equal to freqMaxIdx
			# dst[i, j] = bgr_sum[max_idx] // freq[max_idx]
			dst[i, j] = bgr_sum[max_bin] // freq[freqMaxIdx]
