...
src = cv2.imread("image/bit_test.jpg")
dst = src.copy()
gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
# blur = cv2.GaussianBlur(gray, (0,0), sigmaX=0.5)
canny = cv2.Canny(gray, 100, 350, apertureSize = 3, L2gradient = True)

# lines = cv2.HoughLinesP(canny, 거리단위, 각도단위, 임계값, minLineLength, maxLineGap)
# - 확률적으로 선태하므로, thresh 값을 작게 주어야 한다 ----> 중요
...
lines = cv2.HoughLinesP(canny, 0.8, np.pi / 180, 100, minLineLength = 100, maxLineGap = 50)
# lines = cv2.HoughLinesP(canny, 0.8, np.pi / 180, 100)
